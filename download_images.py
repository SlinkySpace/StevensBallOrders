...
if "remote_image_url" not in df.columns:
    df["remote_image_url"] = df["image_url"]
...
for idx, row in df.iterrows():
    remote_url = str(row.get("remote_image_url", "") or "").strip()
    ...
    url_hash = hashlib.md5(remote_url.encode("utf-8")).hexdigest()[:8]
    filename = f"{safe_slug}_{url_hash}{suffix}"
    output_path = OUTPUT_DIR / filename

    if output_path.exists():
        df.at[idx, "image_url"] = output_path.as_posix()
        already_local += 1
        continue

    try:
        response = session.get(remote_url, timeout=REQUEST_TIMEOUT)
